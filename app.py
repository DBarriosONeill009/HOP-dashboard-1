...
if total_students > 0 and total_delivery_hours > 0 and all(g["Group Size"] > 0 for g in learning_groups):
    ...
    if total_students > 0 and validated_contact_hours > 0:
        ...
        st.write(f"Each validated programme hour requires approximately **{delivery_multiplier:.2f}** hours of group delivery.")
        st.write(f"Weighted repetition across all taught hours: **{weighted_repetition:.2f}x**")
